...
def build_payload(source_settings: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if using_static:
        missing = [field for field in STATIC_FIELDS if not wan1.get(field)]
        if missing:
            raise SystemExit("Source management interface is missing: " + ", ".join(missing))
...
class ManagementIpTask(Task):
    ...
    def preflight(self, ctx: Context) -> None:
        source_settings = with_retry(
            ctx.dashboard.devices.getDeviceManagementInterface,
            ctx.source,
        )
    ...
    def show_plan(self, ctx: Context, plan: Dict[str, Any]) -> None:
        ...
        body = (
            f"Type          : {'Static' if plan['usingStaticIp'] else 'DHCP'}\n"
            f"LAN IPv4      : {plan.get('staticIp', 'DHCP')}\n"
            f"VLAN          : {plan.get('vlan', 'Not configured')}\n"
            f"Subnet mask   : {plan.get('staticSubnetMask', 'DHCP')}\n"
            f"Gateway       : {plan.get('staticGatewayIp', 'DHCP')}\n"
            f"Primary DNS   : {dns[0] if len(dns) >= 1 else 'Not configured'}\n"
            f"Secondary DNS : {dns[1] if len(dns) >= 2 else 'Not configured'}"
        )
        ui.console.print(Panel(body, title="Management IPv4 Plan", border_style="cyan"))
    ...
    def apply(self, ctx: Context, plan: Dict[str, Any]):
        ...
        try:
            ...
            applied.append("wan1")
            ui.write("Updated target management IPv4")
        except Exception as exc:
            failures.append(("wan1", str(exc)))
            ui.fail(f"Management IPv4: {exc}")
    ...
    def verify(self, ctx: Context) -> bool:
        ...
        io.save_report(
            f"{ctx.target}_mgmt_verify_fail",
    ...
    def rollback(self, ctx: Context, applied: List[str]) -> None:
        if not applied:
            ui.safe("Management IPv4 was not changed.")
            return
        original = self.target_backup.get("wan1", {})
        payload = {field: original[field] for field in WRITABLE_FIELDS if field in original}
        try:
            write_once(
                ctx.dashboard.devices.updateDeviceManagementInterface,
                serial=ctx.target,
            ...
            ui.ok("Management IPv4 rollback request completed.")
        except Exception as exc:
            ui.warning(f"Could not restore management IPv4: {exc}")

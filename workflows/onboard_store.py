...
ORGANIZATION_NAME = "BC Liquor Stores"


def _organization(dashboard) -> Dict[str, Any]:
    organizations = with_retry(
    ...
    matches = [
        ...
        if organization.get("name", "").strip().casefold()
        == ORGANIZATION_NAME.casefold()
    ]

    if not matches:
        raise SystemExit(
            f"Meraki organization '{ORGANIZATION_NAME}' was not found."
        )

    if len(matches) > 1:
        raise SystemExit(
            f"Multiple organizations named '{ORGANIZATION_NAME}' were found. "
            "Review the account access manually."
        )
...
def _validate_replacement(
    ...
    """Validates one entered replacement device immediately."""
    device = inventory.get(serial)

    if not device:
        return "unclaimed, claim into network"

    _check_model(device, expected_model, label)

    assigned_network = device.get("networkId")

    if assigned_network and assigned_network != network_id:
        raise SystemExit(
            f"{label} {serial} is already assigned to another Meraki network. "
            "Review the Cloud ID manually."
        )

    if assigned_network == network_id:
        return "already in network"

    return "claimed, add to network"
...
def _collect_replacements(
    ...
    for index, switch in enumerate(existing_switches, start=1):
        serial = ui.ask_cloud_id("New MS130 switch")

        status = _validate_replacement(
            serial=serial,
            inventory=inventory,
            network_id=network_id,
            expected_model=SWITCH_TARGET_MODEL,
            label="New MS130 switch",
        )
    ...
    if ap_count > len(string.ascii_uppercase):
        raise SystemExit(
            "More than 26 access points found. Review the store manually."
        )

    for index in range(ap_count):
        letter = string.ascii_uppercase[index]
        serial = ui.ask_cloud_id(f"New CW9176I access point {letter}")
        ...
        status = _validate_replacement(
            ...
            label=f"New access point {letter}",
        )
    ...
    if len(serials) != len(set(serials)):
        raise SystemExit(
            "The same Cloud ID was entered more than once."
        )

    return replacements

def _is_access_point(device: Dict[str, Any]) -> bool:
    """Identifies Meraki and Catalyst wireless access points."""
    product_type = str(device.get("productType", "")).casefold()
    model = str(device.get("model", "")).upper()

    return (
        product_type == "wireless"
        or model.startswith("MR")
        or model.startswith("CW")
    )
...
def run(dashboard, store_input: str | None = None) -> None:
    ...
    organization = _organization(dashboard)
    ...
    if (
        str(device.get("productType", "")).casefold() == "wireless"
        or str(device.get("model", "")).upper().startswith(("MR", "CW"))
    )]
    ...
    inventory = _inventory_by_serial(
    dashboard,
    organization["id"],
    )
    ...
    if item["status"] in {
        "unclaimed, claim into network",
        "claimed, add to network",
    }
]
